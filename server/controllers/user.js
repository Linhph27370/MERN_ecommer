...
const register = asyncHandler(async (req,res) =>{
    ...
    const user = await User.findOne({ email })
        if (user) throw new Error('User has existed')
            ...
            return res.json({
                success: newUser ? true : false,
                mes: newUser? "Please check your email to active account " : "Somthing went wrong, please try again"
            })
        }
})
const finalRegister = asyncHandler(async(req, res)=>{
    ...
    return res.status(400).json({
        ...
        response: notActivedEmail ? 'Register successfully. Please go login.' : 'something went wrong !'
})
})
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    if (!email) throw new Error('Missing emailsss')
    const user = await User.findOne({ email })
    ...
    return res.status(200).json({
        ...
        mes: rs.response?.includes('OK') ? 'Please, check your email' : 'something wrong with your email! Please try again '
    })
})
...
const updateUserAddress = asyncHandler(async (req, res) => {
    ...
    const { _id } = req.user
    console.log(_id);
    if (!req.body.address) throw new Error('Missing inputs user12')
    console.log(_id);
